...
PDF_CABWArray = zeros(Numb_PDF_Iterations,1);
...
PDF_CABWArray(index_PDF) = mean(SaveTotalTrustArray,'all');

%PDF_RPRAArray(index_PDF) = mean(yLoggedAvergReputAll,'all');
end

% Create a logical index
index_NAN = isnan(PDF_CABWArray);
% Excluding NaN and zeros...
PDF_CABWArrayNew = PDF_CABWArray(~index_NAN); 
% Use logical indexing to create a logical array of elements that match the specified element
CountNaN = sum(index_NAN);
randomIndex = randperm(CountNaN, CountNaN); % Generates a single random index
% Get the randomly selected element
randomElements = PDF_CABWArrayNew(randomIndex);
PDF_CABWArray(index_NAN == 1) = randomElements; 
% randomElement;

% Full version workspace saving we are using is
% "RPRA_AllMethods25Oct2023_PDF_generationWorkspaceV1.mat" 
% The updated version of workspace: RPRA_TEP_IDES_CBAW_WorkspaceUpdated25Oct2023V3.mat 